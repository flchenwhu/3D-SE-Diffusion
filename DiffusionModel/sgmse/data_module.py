...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
            format='default', normalize="noisy", spec_transform=None,inputchoice="noisy",
            stft_kwargs=None, **ignored_kwargs):

        # Read file paths according to file naming format.
        if format == "default":
            self.clean_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
            self.noisy_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
            # if inputchoice == "clean":
            #     self.clean_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
            #     self.noisy_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
        else:
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--base_dir", type=str, choices=("WSJ0-DNS3","audio", "dev22","train100_23","train360_22","mini"),default="/kaggle/input/train360-22/train360_22", help="The base directory of the dataset. Should contain `train`, `valid` and `test` subdirectories, each of which contain `clean` and `noisy` subdirectories.")
        ...
        parser.add_argument("--num_workers", type=int, default=0, help="Number of workers to use for DataLoaders. 4 by default.")
        ...
        parser.add_argument("--normalize", type=str, choices=("clean", "noisy", "not"), default="not", help="Normalize the input waveforms by the clean signal, the noisy signal, or not at all.")
        ...
        # parser.add_argument("--inputchoice", type=str, choices=("clean", "noisy"), default="clean",help="choose which to input,clean or noisy")
        return parser
