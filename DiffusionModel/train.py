...
if __name__ == '__main__':
     ...
     for parser_ in (base_parser, parser):
          parser_.add_argument("--backbone", type=str, choices=BackboneRegistry.get_all_names(), default="dcunet")
          ...
          parser_.add_argument("--no_wandb", action='store_false', help="Turn off logging to W&B, using local default logger instead")
     ...
     if args.num_eval_files:
          checkpoint_callback_estoi = ModelCheckpoint(dirpath=f"logs/{logger.version}", 
               save_top_k=2, monitor="estoi", mode="max", filename='{epoch}-{estoi:.2f}')
          ...
          callbacks += [checkpoint_callback_estoi, checkpoint_callback_si_sdr]
     ...
     trainer = pl.Trainer.from_argparse_args(
          ...
          callbacks=callbacks,
          accelerator="auto"#add this to use cuda,and windows can't support backend=backend,so change C:\Users\123\.conda\envs\pytorch\Lib\site-packages\torch\distributed\distributed_c10d.py lines577 make backend='gloo'
     )

     # Train model
     trainer.fit(model, ckpt_path="logs/0/last.ckpt") #, ckpt_path="logs/6/last.ckpt"