...
def evaluate(model, device, criterion, dataloader):
    ...
    with tqdm(total=len(dataloader) // args.batch_size) as pbar, torch.no_grad():
        for example_num, (x, target) in enumerate(dataloader):
            ...
            x = x.to(device)
            outputs = model(x, torch.tensor([0.]))  #only fasnet/tac
...
def main(args):
    ...
    if args.architecture == 'fasnet':
        ...
        model = MMULB(fft_size=args.fft_size,
                      hop_size=args.hop_size,
                      input_channel=args.input_channel)
    elif args.architecture == 'MSWB':
        model = MSWB(fft_size=args.fft_size,
                      hop_size=args.hop_size,
                      input_channel=args.input_channel)
...
if __name__ == '__main__':
    ...
    #saving parameters
    parser.add_argument('--results_path', type=str, default='RESULTS/tac2mic',
                        help='Folder to write results dicts into')
    parser.add_argument('--checkpoint_dir', type=str, default='RESULTS/tac2mic',
                        help='Folder to write checkpoints into')
    #dataset parameters
    parser.add_argument('--training_predictors_path', type=str, default='DATASETS/processedcut1.912/task1_predictors_train.pkl')
    parser.add_argument('--training_target_path', type=str, default='DATASETS/processedcut1.912/task1_target_train.pkl')
    parser.add_argument('--validation_predictors_path', type=str, default='DATASETS/processedcut1.912/task1_predictors_validation.pkl')
    parser.add_argument('--validation_target_path', type=str, default='DATASETS/processedcut1.912/task1_target_validation.pkl')
    parser.add_argument('--test_predictors_path', type=str, default='DATASETS/processedcut1.912/task1_predictors_test.pkl')
    parser.add_argument('--test_target_path', type=str, default='DATASETS/processedcut1.912/task1_target_test.pkl')
    #training parameters
    ...
    parser.add_argument('--loss', type=str, default="L1",
                        help="L1 or L2")
    #model parameters
    parser.add_argument('--architecture', type=str, default='tac',
                        help="model name")
