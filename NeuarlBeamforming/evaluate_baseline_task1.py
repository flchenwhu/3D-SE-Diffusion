...
def enhance_sound(predictors, model, device, length, overlap):
    ...
    for i in range(len(starts)):
        ...
        # predicted_x = model(cut_x, torch.tensor([0.])) #only fastac
        ...
        for example_num, (x, target) in enumerate(dataloader):
            ...
            if metric is not None:
                ...
                if args.save_sounds_freq is not None:
                    sounds_dir = os.path.join(args.results_path, 'noisy')
                    target_dir = os.path.join(args.results_path, 'clean')
                    ...
                    if count % args.save_sounds_freq == 0:
                        ...
                        sf.write(os.path.join(target_dir, str(example_num) + '.wav'), target, 16000, 'PCM_16')
...
if __name__ == '__main__':
    ...
    #i/o parameters
    parser.add_argument('--model_path', type=str, default='RESULTS/MMUBTask1/066_0.021776.pth')
    parser.add_argument('--results_path', type=str, default='RESULTS/MMUB_2022.12.12/dev')
    parser.add_argument('--save_sounds_freq', type=int, default=1)
    #dataset parameters
    parser.add_argument('--predictors_path', type=str, default='DATASETS/processeddev/task1_predictors_test_uncut.pkl')
    parser.add_argument('--target_path', type=str, default='DATASETS/processeddev/task1_target_test_uncut.pkl')
    parser.add_argument('--sr', type=int, default=16000)
    #reconstruction parameters
    parser.add_argument('--segment_length', type=int, default=30592)
